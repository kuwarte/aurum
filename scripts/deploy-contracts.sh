...
Current blocker:
- contracts/ contains Rust domain/state-machine crates, not deployable Odra modules.
- There is no Odra.toml, no Odra dependency, and no wasm32-unknown-unknown contract output for these packages.

Exact next technical step:
1. Pin the Odra toolchain for this repo.
2. Add Odra contract modules/wrappers that expose the current state-machine behavior as Casper entrypoints.
3. Build release Wasm artifacts for wasm32-unknown-unknown.
